...
const MockExam = ({ isDarkMode, onBack, onStartPractice, onOpenAIReview }: MockExamProps) => {
  ...
  const startExam = async () => {
    ...
    // Build 50 questions proportionally: ~17 per category
    let examQuestions: MockQuestion[] = [];
    CATEGORIES.forEach(cat => {
      const catQs = shuffleArray(MOCK_QUESTIONS[cat] || []);
      examQuestions.push(...catQs.slice(0, 17));
    });

    // Shuffle final set and trim to 50
      ...
      <div className="w-full max-w-2xl mx-auto px-4 py-8 space-y-6">
        ...
        <div className={`${cardBg} rounded-2xl p-8 md:p-11 border shadow-sm space-y-8`}>
          <div className="flex flex-col items-center gap-3 text-center">
            <div className={`w-14 h-14 rounded-2xl flex items-center justify-center ${isDarkMode ? 'bg-purple-900/30' : 'bg-purple-50'}`}>
              <FileText className="w-7 h-7 text-purple-500" />
            </div>
            <div>
              <h1 className={`text-xl font-bold ${textClass}`}>Mock Exam</h1>
              <p className={`text-sm mt-1 leading-relaxed ${subtextClass}`}>
                Simulate actual Civil Service Exam conditions
              </p>
            </div>
          </div>

          <div className={`rounded-xl p-4 space-y-3 ${isDarkMode ? 'bg-zinc-800/60' : 'bg-zinc-50'}`}>
            <p className={`text-xs font-semibold uppercase tracking-wide ${subtextClass}`}>Exam Details</p>
            {[
              { icon: Clock, bg: isDarkMode ? 'bg-purple-900/30' : 'bg-purple-50', color: 'text-purple-500', text: '90-minute timer — real exam pacing' },
              { icon: FileText, bg: isDarkMode ? 'bg-blue-900/30' : 'bg-blue-50', color: 'text-blue-500', text: '50 questions across all 3 categories' },
              { icon: Shield, bg: isDarkMode ? 'bg-red-900/30' : 'bg-red-50', color: 'text-red-500', text: 'No instant feedback — results shown at the end' },
          ...
          <div className={`p-4 rounded-xl border ${isDarkMode ? 'bg-red-900/20 border-red-700/30' : 'bg-red-50 border-red-200'}`}>
            <div className="flex items-start gap-3">
              <AlertTriangle className="w-5 h-5 text-red-500 flex-shrink-0 mt-0.5" />
              <div>
                <p className={`text-sm font-semibold ${textClass}`}>Exam Simulation</p>
                <p className={`text-xs ${isDarkMode ? 'text-red-300' : 'text-red-800'} mt-1`}>
                  This mode simulates real exam conditions. You won't see correct answers until the exam is over. 
                  Make sure you have 90 minutes available before starting.
                </p>
              </div>
            </div>
          </div>

          <button onClick={startExam} className={`w-full py-3.5 rounded-xl font-semibold text-sm ${btnPrimary} transition-colors`}>
            Begin Mock Exam
          </button>
        </div>
      </div>
    ...
    return (
      <div className="w-full max-w-md mx-auto px-4 py-8 space-y-5">
        <div className="text-center space-y-1">
          <h1 className={`text-2xl font-bold ${textClass}`}>{passed ? 'Exam Passed! 🎉' : 'Exam Complete 📋'}</h1>
          <p className={`text-sm ${subtextClass}`}>
            {passed ? 'Excellent performance!' : 'Keep practicing to improve your score'}
          </p>
        </div>

        <div className={`${cardBg} rounded-2xl border p-7 text-center space-y-3`}>
          <p className={`text-xs font-semibold uppercase tracking-wide ${subtextClass}`}>Final Score</p>
          <div className="flex justify-center items-end gap-2">
            <p className={`text-6xl font-bold ${passed ? 'text-emerald-500' : 'text-red-500'}`}>{score}<span className={`text-2xl ${subtextClass}`}>/{questions.length}</span></p>
          </div>
          <p className={`text-sm font-bold ${passed ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400'}`}>
          ...
          <div className={`w-full rounded-full h-2.5 overflow-hidden mt-4 ${isDarkMode ? 'bg-zinc-700' : 'bg-zinc-200'}`}>
            <motion.div
              ...
              className={`h-2.5 rounded-full ${passed ? 'bg-emerald-500' : 'bg-red-500'}`}
            />
          </div>
          <p className="text-xs font-semibold text-emerald-600 dark:text-emerald-400 mt-2">+{score * 10} XP earned</p>
        </div>

        {/* Time analytics */}
        <div className={`grid grid-cols-3 gap-2 ${cardBg} rounded-xl border p-4 text-center`}>
          <div>
          ...
          <div>
            <p className={`text-lg font-bold ${textClass}`}>{answeredCount}/{questions.length}</p>
            <p className={`text-[10px] uppercase font-bold tracking-wider ${subtextClass}`}>Answered</p>
          </div>
        </div>
  ...
  // EXAM IN PROGRESS
  const isTimeLow = timeLeft < 600; // < 10 min
  const isTimeCritical = timeLeft < 300; // < 5 min
    ...
    <div className="w-full max-w-3xl mx-auto px-4 py-6 space-y-6">
      ...
      <div className={`sticky top-4 z-10 py-3.5 px-5 rounded-2xl border shadow-sm backdrop-blur-md ${
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <span className={`text-xs font-medium ${subtextClass} hidden sm:block`}>
              {answeredCount} answered
            </span>
            <div className={`flex items-center gap-2 font-mono text-lg font-bold ${
              isTimeCritical ? 'text-red-500 animate-pulse' : isTimeLow ? 'text-yellow-600 dark:text-yellow-500' : 'text-purple-600 dark:text-purple-400'
            }`}>
        ...
        <div className={`w-full rounded-full h-1.5 mt-3 overflow-hidden ${isDarkMode ? 'bg-zinc-900' : 'bg-zinc-100'}`}>
          <motion.div 
            ...
            className={`h-1.5 rounded-full ${isTimeCritical ? 'bg-red-500' : 'bg-purple-500'}`} 
          />
        </div>
      </div>
      ...
      {showWarning && isTimeCritical && (
        <motion.div
          ...
          className={`p-4 rounded-xl flex items-center gap-3 text-sm font-medium ${isDarkMode ? 'bg-red-900/30 text-red-400 border border-red-700/50' : 'bg-red-50 text-red-600 border border-red-200'}`}
        >
          <AlertTriangle className="w-5 h-5 flex-shrink-0" />
          Less than 5 minutes remaining! Submit your exam soon.
          <button onClick={() => setShowWarning(false)} className="ml-auto text-xs underline font-bold opacity-80 hover:opacity-100">Dismiss</button>
        </motion.div>
      )}
      ...
      <div className="flex gap-3 pt-2">
        ...
        {currentIndex < questions.length - 1 ? (
          <button onClick={nextQuestion} className={`flex-1 py-3.5 rounded-xl font-semibold text-sm flex items-center justify-center gap-2 ${btnPrimary} transition-colors`}>
            Next <ArrowRight className="w-4 h-4" />
          </button>
        ) : (
          <button onClick={finishExam} className={`flex-1 py-3.5 rounded-xl font-semibold text-sm flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white transition-colors`}>
            Submit Exam
          </button>
        )}
      </div>

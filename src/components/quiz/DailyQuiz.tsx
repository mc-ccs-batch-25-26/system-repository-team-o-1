...
const DailyQuiz = ({ isDarkMode, onBack, onStartPractice, onOpenAIReview }: DailyQuizProps) => {
  ...
  const handleAnswer = async (questionId: string, selected: string) => {
    ...
    if (!isCorrect) { saveMistake(q, selected, q.category || 'Unknown'); await getAIExplanation(q, selected); }
  };

  const nextQuestion = () => {
    setShowAI(false); setAiText('');
    if (currentIndex < questions.length - 1) { setCurrentIndex(prev => prev + 1); } else { finishQuiz(); }
  };
  ...
  const finishQuiz = async () => {
    ...
    if (sessionRef.current) {
      await supabase.from('quiz_sessions').update({ score, total_questions: questions.length, quiz_type: 'daily', ended_at: new Date().toISOString() }).eq('id', sessionRef.current);
    }
  ...
  if (phase === 'start') {
    if (isCheckingStatus) return <div className="w-full max-w-3xl mx-auto px-4 py-9 flex justify-center items-center h-64"><div className={`text-sm ${subtextClass}`}>Checking daily status...</div></div>;
      ...
      <div className="w-full max-w-3xl mx-auto px-4 py-9 space-y-6">
        ...
        <div className={`${cardBg} rounded-2xl p-8 md:p-11 border shadow-sm space-y-6 text-center`}>
          <div className="flex flex-col items-center gap-3">
            <div className={`w-16 h-16 rounded-full flex items-center justify-center ${isDarkMode ? 'bg-zinc-800' : 'bg-zinc-100'} ring-8 ${isDarkMode ? 'ring-zinc-800/50' : 'ring-zinc-50'}`}><CheckCircle className="w-8 h-8 text-emerald-500" /></div>
            <div><h1 className={`text-xl font-bold mt-2 ${textClass}`}>You're all done for today!</h1><p className={`text-sm mt-2 leading-relaxed ${subtextClass} max-w-md mx-auto`}>Great job answering your daily quiz. Check back tomorrow for a new set of personalized questions to keep your streak going!</p></div>
          </div>
          <div className={`rounded-xl p-5 border ${isDarkMode ? 'bg-zinc-800/80 border-zinc-700/80' : 'bg-zinc-50 border-zinc-200'}`}><div className="flex items-center justify-center gap-2 mb-1"><Calendar className={`w-4 h-4 ${subtextClass}`} /><span className={`text-sm font-semibold ${textClass}`}>Come back tomorrow</span></div><p className={`text-xs ${subtextClass}`}>Your next daily quiz will be available after midnight.</p></div>
          <button onClick={onBack} className={`w-full py-3.5 rounded-xl font-semibold text-sm ${btnPrimary} transition-colors`}>Return to Hub</button>
        </div>
      </div>
    );
    return (
      <div className="w-full max-w-3xl mx-auto px-4 py-9 space-y-6">
        ...
        <div className={`${cardBg} rounded-2xl p-8 md:p-11 border shadow-sm space-y-6`}>
          <div className="flex flex-col items-center gap-3 text-center">
            <div className={`w-14 h-14 rounded-2xl flex items-center justify-center ${isDarkMode ? 'bg-yellow-900/30' : 'bg-yellow-50'}`}><Zap className="w-7 h-7 text-yellow-500" /></div>
            <div><h1 className={`text-xl font-bold ${textClass}`}>Daily Adaptive Quiz</h1><p className={`text-sm mt-1 leading-relaxed ${subtextClass}`}>5 questions targeting your weak areas</p></div>
          </div>
          <div className={`rounded-xl p-4 space-y-3 ${isDarkMode ? 'bg-zinc-800/60' : 'bg-zinc-50'}`}>
            ...
            {[
              ...
              <div key={text} className="flex items-center gap-3"><div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${bg}`}><Icon className={`w-4 h-4 ${color}`} /></div><div className={`text-sm leading-relaxed ${textClass}`}>{text}</div></div>
            ))}
          </div>
          <button onClick={startQuiz} className={`w-full py-3.5 rounded-xl font-semibold text-sm ${btnPrimary} transition-colors`}>Start Daily Quiz</button>
        </div>
      </div>
    );
  }
  ...
  if (phase === 'results') {
    ...
    return (
      <div className="w-full max-w-xl mx-auto px-4 py-8 space-y-5">
        <div className="text-center space-y-1"><h1 className={`text-2xl font-bold ${textClass}`}>Quiz Complete! 🎉</h1><p className={`text-sm ${subtextClass}`}>Here's how you did</p></div>
        <div className={`${cardBg} rounded-2xl border p-7 text-center space-y-3`}>
          ...
          <div className={`w-full rounded-full h-2.5 overflow-hidden ${isDarkMode ? 'bg-zinc-700' : 'bg-zinc-300'}`}><motion.div initial={{ width: 0 }} animate={{ width: `${(score / questions.length) * 100}%` }} transition={{ duration: 0.8, ease: 'easeOut' }} className="h-2.5 rounded-full bg-blue-500" /></div>
          ...
          {Object.entries(categoryStats).map(([cat, stats]) => {
            ...
            return (
              <div key={cat} className={`${cardBg} rounded-xl border p-4 space-y-2.5`}>
                <div className="flex items-center justify-between"><span className={`text-xs font-semibold px-2.5 py-0.5 rounded-full ${catCol.pill}`}>{cat}</span><span className={`text-sm font-bold ${col.text}`}>{stats.correct}/{stats.total}</span></div>
                <div className={`w-full rounded-full h-1.5 overflow-hidden ${isDarkMode ? 'bg-zinc-800' : 'bg-zinc-100'}`}><motion.div initial={{ width: 0 }} animate={{ width: `${(stats.correct / stats.total) * 100}%` }} transition={{ duration: 0.6, ease: 'easeOut' }} className={`h-1.5 rounded-full ${col.bar}`} /></div>
              </div>
            );
          })}
        </div>
        ...
        <div className="flex gap-3 pt-2">
          <button onClick={() => window.location.href = '/'} className={`w-full py-3.5 rounded-xl font-semibold text-sm ${btnPrimary} transition-colors`}>Back to Dashboard</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="w-full max-w-3xl mx-auto px-4 py-8 space-y-5">
      <button onClick={onBack} className={`flex items-center gap-2 text-sm font-medium ${subtextClass} hover:${textClass} transition-colors`}><ArrowLeft className="w-5 h-5" /> Exit Quiz</button>
      <div className="space-y-2">
        <div className={`flex justify-between text-xs font-medium ${subtextClass}`}><span>Question {currentIndex + 1} of {questions.length}</span></div>
        <div className={`w-full rounded-full h-1.5 overflow-hidden ${isDarkMode ? 'bg-zinc-800' : 'bg-zinc-300'}`}><motion.div animate={{ width: `${progress}%` }} transition={{ duration: 0.3 }} className="h-1.5 rounded-full bg-blue-500" /></div>
        <div className="flex justify-center gap-1.5 pt-1">{questions.map((q, i) => (<div key={q.id} className={`rounded-full transition-all duration-200 ${i === currentIndex ? 'w-4 h-1.5 bg-blue-500' : answers[q.id] ? 'w-1.5 h-1.5 bg-blue-300 dark:bg-blue-800' : 'w-1.5 h-1.5 bg-zinc-300 dark:bg-zinc-700'}`} />))}</div>
      </div>
      ...
      <div className="pt-2">{currentQuestion && answers[currentQuestion.id] && (
        <button onClick={nextQuestion} className={`w-full py-3.5 rounded-xl font-semibold text-sm flex items-center justify-center gap-2 ${btnPrimary} transition-colors`}>{currentIndex < questions.length - 1 ? 'Next Question' : 'Finish Quiz'}<ArrowRight className="w-4 h-4" /></button>
      )}</div>
    </div>
  );
};
...
const WeakAreasFocus = ({ isDarkMode }: { isDarkMode: boolean }) => {
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      if (perf && perf.length > 0) {
        setWeakCats(perf.filter((p: any) => (p.accuracy_rate || 0) < 70).slice(0, 2).map((p: any) => p.categories?.name).filter(Boolean));
      } else {
        ...
        if (pretestData && pretestData.length > 0) setWeakCats(pretestData.map((p: any) => p.categories?.name).filter(Boolean));
      }
    };
    fetch();
  }, []);

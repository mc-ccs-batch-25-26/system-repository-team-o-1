...
import { Flame, Target, TrendingUp, Award, Zap, BookOpen, Lock, CheckCircle2 } from 'lucide-react';
    ...
    useEffect(() => {
        const loadDashboard = async () => {
            ...
            const [profileRes, perfRes, sessionRes, mockRes, categoriesRes] = await Promise.all([
                ...
                supabase.from('quiz_sessions').select('score, total_questions, quiz_type, ended_at').eq('user_id', user.id).eq('is_pretest', false).order('ended_at', { ascending: false }).limit(1).maybeSingle(),
        ...
        <div className="flex flex-col items-center gap-1 shrink-0">
            ...
            <div className="flex items-center gap-1.5">
                    ...
                    return (
                        <div key={i} className="flex flex-col items-center gap-0.5">
                            <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold transition-all duration-300 ${
                                isFilled ? 'text-white shadow-sm' : 'bg-zinc-100 dark:bg-zinc-800 text-zinc-400'
                            }`}
                    ...
                    <div className="flex flex-col sm:flex-row items-center gap-6 p-6">
                        ...
                        <div className="flex-1 text-center sm:text-left space-y-3">
                            ...
                            <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-1 leading-relaxed max-w-sm">
                                {readinessMessage(civiquestUser.readiness, civiquestUser.pretestDone)}
                            </p>
                            <div className="flex gap-2.5 justify-center sm:justify-start flex-wrap">
                                <button onClick={() => navigate(civiquestUser.pretestDone ? '/quizzes?mode=daily' : '/pretest')}
                                    className="flex items-center gap-2 px-5 py-2.5 rounded-xl font-semibold text-sm bg-blue-600 hover:bg-blue-700 active:scale-[0.98] text-white transition-all duration-200 shadow-sm hover:shadow-md">
                                    <Zap className="w-4 h-4" />{civiquestUser.pretestDone ? 'Start Daily Quiz' : 'Take Pre-Test'}
                                </button>
                                <button onClick={() => navigate('/lessons')}
                                    className="flex items-center gap-2 px-5 py-2.5 rounded-xl font-semibold text-sm border border-zinc-200 dark:border-zinc-700 text-zinc-600 dark:text-zinc-300 hover:bg-zinc-50 dark:hover:bg-zinc-800 hover:border-zinc-300 dark:hover:border-zinc-600 transition-all duration-200">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <motion.div variants={fadeUp}
                        ...
                        <div className="flex items-center gap-2 px-5 py-4 border-b border-zinc-100 dark:border-zinc-800">
                            <Target className="w-4 h-4 text-rose-500" /><h3 className="text-sm font-semibold text-zinc-900 dark:text-white">Weak Areas</h3>
                        </div>
                        <div className="p-5">
                            {weakAreas.length === 0 ? (
                                <div className="flex flex-col items-center py-6 text-center gap-2">
                                    {civiquestUser.pretestDone ? (
                                        <><CheckCircle2 className="w-7 h-7 text-emerald-500" /><p className="text-sm font-medium text-zinc-600 dark:text-zinc-400">All weak areas resolved!</p></>
                                    ) : (
                                        <><Target className="w-7 h-7 text-zinc-300 dark:text-zinc-600" /><p className="text-sm font-medium text-zinc-600 dark:text-zinc-400">Complete the pre-test</p></>
                                    )}
                                </div>
                            ) : (
                                        ...
                                        return (
                                            <div key={area.category} className="space-y-1.5">
                                                <div className="flex items-center justify-between">
                                                    <span className="text-xs font-medium text-zinc-700 dark:text-zinc-300">{area.category}</span>
                                                    <span className={`text-xs font-bold tabular-nums ${col.text}`}>{area.accuracy}%</span>
                                                </div>
                    ...
                    <motion.div variants={fadeUp}
                        className="rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 shadow-sm overflow-hidden">
                        <div className="flex items-center gap-2 px-5 py-4 border-b border-zinc-100 dark:border-zinc-800">
                            <TrendingUp className="w-4 h-4 text-emerald-500" /><h3 className="text-sm font-semibold text-zinc-900 dark:text-white">Recent Quizzes</h3>
                        </div>
                        <div className="p-5">
                            {recentQuiz.total === 0 ? (
                                <div className="flex flex-col items-center py-6 text-center gap-2">
                                    <TrendingUp className="w-7 h-7 text-zinc-300 dark:text-zinc-600" /><p className="text-sm font-medium text-zinc-600 dark:text-zinc-400">No quizzes yet</p>
                                </div>
                            ) : (
                                <div className="space-y-4">
                                    <div className="flex items-center justify-between">
                                        <span className="text-xs text-zinc-400 dark:text-zinc-500">Last Quiz</span>
                                        <div className="text-right"><p className="text-xs font-semibold text-zinc-700 dark:text-zinc-300">{recentQuiz.quizType || 'Quiz'}</p>{recentQuiz.date !== 'Not yet' && <p className="text-[11px] text-zinc-400 dark:text-zinc-500">{recentQuiz.date}</p>}</div>
                                    </div>
                                    <div className="flex items-center justify-between">
                                        <span className="text-xs text-zinc-400 dark:text-zinc-500">Score</span>
                                        <span className="text-sm font-bold text-zinc-800 dark:text-zinc-200 tabular-nums">{recentQuiz.score}{recentQuiz.total > 0 && <span className="font-normal text-zinc-400">/{recentQuiz.total}</span>}</span>
                                    </div>
                                    <div className="flex items-center justify-between">
                                        <span className="text-xs text-zinc-400 dark:text-zinc-500">Accuracy</span>
                                        <span className={`text-xs font-bold px-2 py-0.5 rounded-md ${recentColor.badge}`}>{recentQuiz.accuracy}%</span>
                                    </div>
                ...
                <motion.div variants={fadeUp}
                    ...
                    <div className="flex items-center gap-2 px-5 py-4 border-b border-zinc-100 dark:border-zinc-800">
                        <Award className="w-4 h-4 text-amber-500" /><h3 className="text-sm font-semibold text-zinc-900 dark:text-white">Recent Achievements</h3>
                    </div>
                    <div className="p-5">
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                            {[
                                { name: 'First Quiz', unlocked: civiquestUser.pretestDone, icon: '🎯', desc: 'Take your first quiz' },
                                { name: '3-Day Streak', unlocked: civiquestUser.streak >= 3, icon: '🔥', desc: '3 days in a row' },
                                { name: 'Perfect Score', unlocked: false, icon: '⭐', desc: '100% on any quiz' },
                                { name: '10 Quizzes', unlocked: false, icon: '📚', desc: 'Complete 10 quizzes' },
                            ].map((badge) => (
